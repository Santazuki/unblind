...
详见 `docs/project-prepare-md/多agent协作开发unblind.md`。双 pipeline 6 角色：
- **Part 1**: Architect → Developer + Reviewer（交叉审查）
- **Part 2**: Security Lead → QA Engineer → Reliability Engineer（循环至 CLEAN）
- **自动触发**：说"多 agent"即启动完整双 pipeline，不漏角色。
- **交叉审查**：Developer A 的代码由 Reviewer B 审查。
